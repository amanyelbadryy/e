...
import { generate100LetterQuizQuestions, LetterQuizQuestion } from '../../data/games/letterQuizQuestions';
...
interface LetterQuizSectionProps {
  onBack: () => void;
  onGoHome?: () => void;
}

export const LetterQuizSection: React.FC<LetterQuizSectionProps> = ({ onBack, onGoHome }) => {
  const [questions, setQuestions] = useState<LetterQuizQuestion[]>(() => generate100LetterQuizQuestions());
  ...
  useEffect(() => {
    if (!isProcessingAnswer && currentQuestion && currentQuestion.audio) {
  ...
  const handleRestartQuiz = () => {
    ...
    setQuestions(generate100LetterQuizQuestions());
    setCurrentIndex(0);
  ...
  const handleSelectOption = async (option: string) => {
    ...
    // Wait for the feedback audio to finish completely via onended
  ...
  return (
    <GameShell
      title="🎯 اختبر نفسك في الحروف"
      description="اختبار شامل مكون من 100 سؤال متنوع لتقييم معرفتك بالحروف والكلمات والحركات"
      icon="🎯"
        ...
        >
          ...
          <div className="space-y-3">
            <div className="inline-flex items-center gap-2 bg-teal-100 text-teal-950 px-4 py-1.5 rounded-full font-black text-xs border border-teal-300">
              ...
              <span>اختبار الحروف التفاعلي</span>
            </div>
          ...
          {/* Emoji / Picture Display if present */}
          {currentQuestion.emoji && (
            <div className="py-2">
              <span className="text-6xl md:text-7xl drop-shadow-md inline-block hover:scale-110 transition-transform">
                {currentQuestion.emoji}
              </span>
            </div>
          )}

          {/* Audio Play Button if audio is present */}
          {currentQuestion.audio && (
            <div className="py-2 flex justify-center">
              <button
                type="button"
                onClick={() => playMP3(currentQuestion.audio!)}
            ...
            {currentQuestion.options.map((option, idx) => {
              ...
              return (
                <button
                  ...
                  className={`p-5 rounded-3xl border-4 font-black transition-all text-2xl flex items-center justify-center min-h-[85px] cursor-pointer ${btnStyle}`}
                >
