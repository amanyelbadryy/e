...
import { getMixedListeningDeck, ListeningQuestion } from '../../data/games/listeningQuestions';
...
export const ListeningGame: React.FC<ListeningGameProps> = ({ onBack, onGoHome }) => {
  const TOTAL_QUESTIONS = 10;
  const [questionDeck, setQuestionDeck] = useState<ListeningQuestion[]>(() =>
    getMixedListeningDeck(TOTAL_QUESTIONS)
  );
  ...
  const handleSelect = (choice: string) => {
    ...
    if (choice === currentTarget.correctText) {
      setIsCorrect(true);
  ...
  const handleRestart = () => {
    setQuestionDeck(getMixedListeningDeck(TOTAL_QUESTIONS));
    setQuestionIndex(0);
    setScore(0);
    setCorrectCount(0);
    setWrongCount(0);
          ...
          <div className="min-h-[220px] flex items-center justify-center py-2">
            <div className="bg-[#FAF5FF] p-6 md:p-8 rounded-[32px] border-4 border-[#F3E8FF] space-y-4 max-w-sm w-full shadow-inner">
              ...
              >
                ...
                <span>اِسْتَمِعْ لِلصَّوْتِ الآن</span>
              </button>
            </div>
          </div>
              ...
              >
                {isCorrect ? (
                  <div className="p-3.5 bg-[#DCFCE7] text-emerald-950 rounded-2xl border border-[#86EFAC] font-black text-lg flex items-center justify-center gap-2">
                    ...
                    <span>إجابة صحيحة! الصوت الصحيح هو ({currentTarget.correctText}) 🌟</span>
                  </div>
                ) : (
                  <div className="p-3.5 bg-rose-100 text-rose-950 rounded-2xl border border-rose-300 font-black text-lg flex items-center justify-center gap-2">
                    ...
                    <span>حاول مجدداً! الإجابة الصحيحة هي ({currentTarget.correctText})</span>
                  </div>
                )}
