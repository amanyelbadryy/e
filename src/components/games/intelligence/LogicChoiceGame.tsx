...
import { getLogicQuestions, LogicQuestion } from '../../../data/games/logicQuestions';
...
interface LogicChoiceGameProps {
  onBack: () => void;
  onGoHome?: () => void;
}

export const LogicChoiceGame: React.FC<LogicChoiceGameProps> = ({ onBack, onGoHome }) => {
  ...
  const setupNewGame = () => {
    const deck = getLogicQuestions(10);
    setQuestions(deck);
    setCurrentIndex(0);
    setScore(0);
    setCorrectCount(0);
  ...
  const handleOptionClick = (opt: { emoji: string; text: string }) => {
    ...
    >
      <div className="bg-white rounded-[40px] p-6 md:p-8 border-4 border-[#F1F5F9] shadow-xl space-y-8 text-center">
        <div className="inline-flex items-center gap-2 bg-[#FEF3C7] text-[#78350F] px-4 py-1.5 rounded-full border border-[#FDE68A] text-xs font-black">
          ...
          <span>مَوْقِفٌ بَسِيطٌ.. فَكِّرْ جَيِّداً ثُمَّ اخْتَرِ التَّصَرُّفَ المُنَاسِبَ!</span>
        </div>
        ...
        <motion.div
          ...
          className="bg-gradient-to-br from-[#FEFCE8] to-[#FEF3C7] p-6 md:p-8 rounded-3xl border-4 border-[#FDE68A] shadow-inner space-y-4 max-w-2xl mx-auto"
        >
          <div className="w-16 h-16 bg-white rounded-2xl mx-auto flex items-center justify-center text-3xl shadow-md border-2 border-[#F59E0B]">
            <Lightbulb className="w-8 h-8 text-[#D97706]" />
          </div>
          <h3 className="text-2xl md:text-3xl font-black text-[#78350F] leading-relaxed">
            "{currentQ.situation}"
          </h3>
        </motion.div>
