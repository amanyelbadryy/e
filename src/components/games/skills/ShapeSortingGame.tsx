...
import { getShapeSortingQuestions, ShapeSortingQuestion } from '../../../data/games/classificationQuestions';
import { playPositiveFeedback, playNegativeFeedback } from '../../../utils/gameHelpers';
import { motion } from 'motion/react';
import { Shapes, Sparkles, CheckCircle2, XCircle } from 'lucide-react';
...
export const ShapeSortingGame: React.FC<ShapeSortingGameProps> = ({ onBack, onGoHome }) => {
  const [questions, setQuestions] = useState<ShapeSortingQuestion[]>([]);
  ...
  const setupNewGame = () => {
    const deck = getShapeSortingQuestions(10);
    setQuestions(deck);
    setCurrentIndex(0);
    setScore(0);
    setCorrectCount(0);
      ...
      <div className="bg-white rounded-[40px] p-6 md:p-8 border-4 border-[#F1F5F9] shadow-xl space-y-8 text-center">
        ...
        <div className="space-y-2">
          <div className="inline-flex items-center gap-2 bg-[#E0F2FE] text-[#0369A1] px-4 py-1.5 rounded-full border border-[#BAE6FD] text-xs font-black">
            <Shapes className="w-4 h-4 text-[#0284C7]" />
            <span>عَالَمُ الأَشْكَالِ الهَنْدَسِيَّةِ</span>
          </div>
          <h3 className="text-2xl md:text-3xl font-black text-[#1E293B]">
            {currentQ.targetShapeTitle}
          </h3>
        </div>
