...
import { getSizeOrderingQuestions, SizeOrderingQuestion } from '../../../data/games/classificationQuestions';
...
export const SizeOrderingGame: React.FC<SizeOrderingGameProps> = ({ onBack, onGoHome }) => {
  const [questions, setQuestions] = useState<SizeOrderingQuestion[]>([]);
  ...
  const setupNewGame = () => {
    const deck = getSizeOrderingQuestions(10);
    setQuestions(deck);
    setCurrentIndex(0);
    setScore(0);
    setCorrectCount(0);
      ...
      <div className="bg-white rounded-[40px] p-6 md:p-8 border-4 border-[#F1F5F9] shadow-xl space-y-8 text-center">
        ...
        <div className="space-y-2">
          <div className="inline-flex items-center gap-2 bg-[#FEF3C7] text-[#78350F] px-4 py-1.5 rounded-full border border-[#FDE68A] text-xs font-black">
            ...
            <span>مُقَارَنَةُ الأَحْجَامِ وَالأَبْعَادِ</span>
          </div>
