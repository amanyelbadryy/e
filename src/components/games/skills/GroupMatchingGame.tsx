...
import { getGroupMatchingQuestions, GroupMatchingQuestion } from '../../../data/games/classificationQuestions';
import { playPositiveFeedback, playNegativeFeedback } from '../../../utils/gameHelpers';
import { motion } from 'motion/react';
import { Sparkles, CheckCircle2, XCircle } from 'lucide-react';

interface GroupMatchingGameProps {
  onBack: () => void;
  onGoHome?: () => void;
}

export const GroupMatchingGame: React.FC<GroupMatchingGameProps> = ({ onBack, onGoHome }) => {
  const [questions, setQuestions] = useState<GroupMatchingQuestion[]>([]);
  ...
  const setupNewGame = () => {
    const deck = getGroupMatchingQuestions(10);
    setQuestions(deck);
    setCurrentIndex(0);
    setScore(0);
    setCorrectCount(0);
      ...
      <div className="bg-white rounded-[40px] p-6 md:p-8 border-4 border-[#F1F5F9] shadow-xl space-y-8 text-center">
        ...
        <div className="space-y-2">
          <div className="inline-flex items-center gap-2 bg-[#FAF5FF] text-[#6B21A8] px-4 py-1.5 rounded-full border border-[#F3E8FF] text-xs font-black">
            ...
            <span>اخْتَرِ العُنْصُرَ الَّذِي يُنَاسِبُ هَذِهِ المَجْمُوعَةَ!</span>
          </div>
        ...
        {/* Group Banner Display */}
        <motion.div
          key={currentQ.id}
          initial={{ scale: 0.9, opacity: 0 }}
          animate={{ scale: 1, opacity: 1 }}
          className="bg-[#FAF5FF] p-6 rounded-3xl border-4 border-[#F3E8FF] shadow-inner max-w-md mx-auto flex items-center justify-center gap-4"
        >
          <span className="text-6xl">{currentQ.groupEmoji}</span>
          <span className="text-xl font-black text-[#6B21A8]">
            أي عنصر ينتمي لهذه العائلة؟
          </span>
        </motion.div>
