...
import { getLetterPictureDeck, LetterPictureQuestion } from '../../data/games/letterPictureQuestions';
...
interface LetterPictureGameProps {
  onBack: () => void;
  onGoHome?: () => void;
}

export const LetterPictureGame: React.FC<LetterPictureGameProps> = ({ onBack, onGoHome }) => {
  const TOTAL_QUESTIONS = 10;
  const [questionDeck, setQuestionDeck] = useState<LetterPictureQuestion[]>(() =>
    getLetterPictureDeck(TOTAL_QUESTIONS)
  );
  ...
  const handleSelect = (letterChoice: string) => {
    ...
    if (letterChoice === currentItem.correctLetter) {
      setIsCorrect(true);
  ...
  const handleRestart = () => {
    setQuestionDeck(getLetterPictureDeck(TOTAL_QUESTIONS));
    setQuestionIndex(0);
    setScore(0);
    setCorrectCount(0);
    setWrongCount(0);
        ...
        >
          ...
          <div className="space-y-2 min-h-[64px]">
            ...
            <h3 className="text-2xl md:text-3xl font-black text-[#1E293B]">
              ما هو الحرف الأول لـ ({currentItem.word})؟
            </h3>
          </div>

          {/* Question Area - Fixed height & centered element */}
          <div className="min-h-[220px] flex items-center justify-center py-2">
            <div className="bg-[#ECFDF5] p-6 rounded-[32px] border-4 border-[#A7F3D0] space-y-3 max-w-sm w-full mx-auto shadow-inner">
              <div className="text-7xl">{currentItem.emoji}</div>
              <div className="text-4xl font-black text-[#065F46]">{currentItem.word}</div>
              <button
                onClick={() => playMP3(currentItem.audio)}
                className="bg-[#10B981] text-white px-6 py-2.5 rounded-full text-base font-black inline-flex items-center gap-2 shadow-md border-b-4 border-[#047857] hover:bg-[#059669] cursor-pointer active:scale-95 transition-transform"
              >
              ...
              >
                {isCorrect ? (
                  <div className="p-3.5 bg-[#DCFCE7] text-emerald-950 rounded-2xl border border-[#86EFAC] font-black text-lg flex items-center justify-center gap-2">
                    ...
                    <span>إجابة صحيحة! تبدأ بحرف ({currentItem.correctLetter}) 🌟</span>
                  </div>
                ) : (
                  <div className="p-3.5 bg-rose-100 text-rose-950 rounded-2xl border border-rose-300 font-black text-lg flex items-center justify-center gap-2">
                    ...
                    <span>حاول مجدداً! الحرف الصحيح هو ({currentItem.correctLetter})</span>
                  </div>
                )}
