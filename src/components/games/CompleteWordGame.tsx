...
import { getCompleteWordDeck, CompleteWordQuestion } from '../../data/games/completeWordQuestions';
...
export const CompleteWordGame: React.FC<CompleteWordGameProps> = ({ onBack, onGoHome }) => {
  const TOTAL_QUESTIONS = 10;
  const [questionDeck, setQuestionDeck] = useState<CompleteWordQuestion[]>(() =>
    getCompleteWordDeck(TOTAL_QUESTIONS)
  );
  ...
  const handleSelect = (letterChoice: string) => {
    ...
    if (letterChoice === currentItem.missingLetter) {
      setIsCorrect(true);
  ...
  const handleRestart = () => {
    setQuestionDeck(getCompleteWordDeck(TOTAL_QUESTIONS));
    setQuestionIndex(0);
    setScore(0);
    setCorrectCount(0);
    setWrongCount(0);
        ...
        >
          ...
          <div className="space-y-2 min-h-[64px]">
            ...
            <h3 className="text-2xl md:text-3xl font-black text-[#1E293B]">
              اختر الحرف الناقص لتكتمل الكلمة!
            </h3>
          </div>

          {/* Question Area - Fixed height & centered element */}
          <div className="min-h-[220px] flex items-center justify-center py-2">
            <div className="bg-[#FAF5FF] p-6 rounded-[32px] border-4 border-[#F3E8FF] space-y-4 max-w-sm w-full mx-auto shadow-inner">
              <div className="text-7xl">{currentItem.emoji}</div>
              <div className="text-5xl font-black text-[#6B21A8] tracking-wider">
                {currentItem.displayWord}
              </div>
              <button
                onClick={() => playMP3(currentItem.audio)}
                className="bg-[#A855F7] text-white px-6 py-2.5 rounded-full text-base font-black inline-flex items-center gap-2 shadow-md border-b-4 border-[#7E22CE] hover:bg-[#9333EA] cursor-pointer active:scale-95 transition-transform"
              >
              ...
              >
                {isCorrect ? (
                  <div className="p-3.5 bg-[#DCFCE7] text-emerald-950 rounded-2xl border border-[#86EFAC] font-black text-lg flex items-center justify-center gap-2">
                    ...
                    <span>إجابة صحيحة! الكلمة الكاملة هي ({currentItem.fullWord}) 🌟</span>
                  </div>
                ) : (
                  <div className="p-3.5 bg-rose-100 text-rose-950 rounded-2xl border border-rose-300 font-black text-lg flex items-center justify-center gap-2">
                    ...
                    <span>حاول مجدداً! الحرف الناقص هو ({currentItem.missingLetter})</span>
                  </div>
                )}
